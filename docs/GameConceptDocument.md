...
**Author:** [ … ]  
**Studio:** [ … ]  
**Version:** 2.0 · Simulation-first pivot — emergent society-sim · refined Pillar 1 · autonomous factions · integrity axis · 2026-06-04
...
**Pillar 1 — "The world never lies."** The *simulation* is sacrosanct: ground truth is consistent and complete — the world always *knows* what really happened — and every contradiction is real and meaningful. **Solvability is tiered (v2.0):** every case has a **provable factual core** reachable from obtainable records, and a deliberate *marquee minority* are **underdetermined at the final link** — evidence converges on a best, defensible theory but stops short of proof (the city's Zodiacs); what is never allowed is a **blank wall**, so every case always yields at least a reachable theory. Records, however, *can* lie — a statement, autopsy, or file may be wrong, partial, biased, or falsified — but every such falsehood is itself a **deterministic, discoverable fact**, and the LLM only ever renders records faithfully (including their built-in falsehoods), never inventing truth. *Rules out:* hallucinated facts, ungrounded red herrings, unsolvable "gotcha" mysteries. *(Refined in §2.4 and §5.1: ground truth vs. the record layer; solvability tiers — a provable core vs. honest underdetermination, never a blank wall.)*
...
**How performance is measured.** There is still **no score and no *morality* / karma meter** (the v2.0 **Integrity axis** below is deliberately *not* one — it tracks how *compromised and owned* the detective is, a liability and a lever against him, never a "goodness" score) — a number that rewarded the "right" choice would collapse the game's moral greyness into an optimization target. Performance lives on **three** planes (v2.0 adds Integrity):
...
- **Integrity — the detective's own compromise (v2.0).** A tracked **Integrity / corruption** axis for the player-character himself, governed by the same incentive model as every NPC (§2.5): favors taken, debts owed, corners cut, secret allegiances formed under pressure. It is **not a virtue score** — low Integrity is not "losing," it is *being owned*: it unlocks the corrupt path's powers (access, money, protection) while making the detective's **own filings carry hidden fidelity** (he can bury his findings) and handing factions **leverage** over him. Branching states (clean → compromised → owned) gate available endings; greyness is preserved by keeping the corrupt path genuinely powerful and never displaying Integrity as a moral grade.
...
**v2.0 — from authored factions to an autonomous society-sim.** The authored layer below (Interests, Temperament, Members, ally/rival ties) is **retained as the seed**, but it now feeds a continuously-running simulation rather than a per-case reaction engine. Seven authored **blocs** — Press · Political Machine · Organized Crime · **Unorganized Crime** · Law Enforcement · Reform/Civic · Business/Industry — each hold **sub-factions** (newspapers, gangs, parties, outfits) which hold **members**. Across the city's simulated history (1920s → the ~1950 present) and onward in live play, factions pursue agendas, fight, deal, rise, and fall:
- **Allegiance — a portfolio, not a membership.** Every actor (faction *or* person, **including the detective**) holds allegiance ties, each with a **strength**, a **public/secret** flag, a role/rank, and an append-only **history**. Affiliation is **fluid**: an actor switches when **incentive − cost** crosses a threshold (material / power / survival / coercion / loyalty, against bonds, fear of retaliation, reputation). One model unifies witness-flipping, corruption, moles, and defection cascades — a bought cop is *public: Law Enforcement + secret: a gang*, and uncovering that secret tie can break a record open.
- **Relationships & Pacts — a living diplomacy graph.** Any two factions (**across or within** a bloc) carry a relationship (allied ↔ neutral ↔ at-war) + trust + a grievance ledger; **Pacts** are first-class temporal objects (non-aggression / territory / tribute / alliance) that **form, strain, renegotiate, and break** each tick — the Organized ↔ Unorganized turf war and its shifting understandings emerge here. Bloc membership is itself fluid (a crew that organizes crosses into Organized Crime; an outfit that loses discipline falls out).
- **The sim authors the record.** Faction actions **emit Registry records** whose fidelity / motive / provenance are set by the acting faction's integrity — Pillar 1's distortions finally have **agents** (crime distorts by muscle, the machine by authority, the press by circulation). The governance **audit log** doubles as the factions' **sensor net**: investigating a case raises **Heat**, which the guilty faction reads and answers with **active cover-up** — the observer effect (looking at a case can degrade its trail).

*(This stays within §2.4's no-grade rule and Pillar 2 — factions pressure, court, and mislead, never interpret the case. The full seven-bloc roster, the allegiance/incentive engine, and the pact tick live in the re-laid faction-simulation plans; the authored fields below remain the seed the sim grows from.)*
...
**Level structure (authored / procedural / hybrid):** **Hybrid.** Authored **case grammars** (templates of case shapes and motive/crime patterns) and the overarching **systemic-secret backbone** provide the skeleton; a procedural generator fills them with specific entities, events, relationships, and records. This yields controllable, guaranteed-interesting structures *plus* procedural variety and interlinking. (Concrete generator technology is deferred to §14, where **simulation-first emergence** — simulating lives and letting crimes emerge — is now **adopted as the core generation model (v2.0)**: a deterministic society-sim (§2.5) simulates lives, factions, and crimes forward across the city's history, and **cold cases are the unsolved residue of that simulated past** — their records the distorted exhaust the sim left behind. Authored case grammars + the secret backbone **seed and constrain** the sim; the solvability checker still gates **every** realized case (emergence never costs Pillar 1); and the sim runs coarse, **realizing people and cases lazily, on touch**, deterministically. See §14.)
...
#### The White Whale *(underdetermined — v2.0)*
- **Shape:** an infamous, over-documented case (the city's Zodiac / Cooper) — multiple live suspects, competing theories, no surviving conclusive link.
- **Lie-pattern:** *not* an erasure — the case is evidence-**rich**; the one load-bearing corroboration was destroyed or never existed, so the record **underdetermines** the truth. The player builds and defends a best theory and may close it **by theory**, never by proof.
- **District lean:** anywhere; reserved for marquee / secret-backbone cases. A strict minority — never a blank wall (Pillar 1, §1.2).
...
**v2.0 reconciliation (2026-06-04 scoping call):** the "secret" is now **emergent, not authored** — it's the apex of the simulated corruption graph + the deepest sealed record (the central White Whale), so Pillar 4 is satisfied without an authored backbone, and **case interlinking emerges** from the shared sim (see §5.1). The endless, self-refreshing world has no single forced finale; instead the **detective's Integrity arc + accumulated faction Standing** drive the branching endings (crusader / owned / burned-out). The LLM renderer and the §13 audio/visual layer are deferred polish, not active scope.
...
> ✅ **Decided 2026-06-02**, after the design took shape (as intended). Summary: **TypeScript end-to-end**, a **web app** (HTML/Canvas + PixiJS; desktop via Tauri later), with a **pure deterministic core engine** and the **LLM as a cached, swappable presentation adapter** (with a no-LLM template fallback). Rationale and risks below. **(v2.0 update):** the deterministic core is now a **society-sim** (simulation-first promoted from stretch to core), and the Registry gains a **persistence + server tier** — a Node/Fastify API over **Prisma** (SQLite in dev → **Postgres** in prod, with **Apache AGE** for the faction / allegiance / pact graph and **RLS** for access tiers). The client stays the desk/Board UI; the LLM stays a cached, swappable renderer; the solver still gates every case.
...
- **Simulation-first generation (v2.0 — now core, no longer a stretch):** the highest-risk piece of the pivot; still built **in isolation and on rails** — authored grammars seed it and the solver gates every realized case, so emergence never blocks solvability. De-risk by growing it in layers (data model → tick + scripted agendas → emergent autonomy), each shippable behind the gate.
...
|-----|--------------------------------------------------------------------------|
|2026-06-04|**Scoping call — three "unplanned" GCD systems cleared off the active backlog (v2.0).** Evaluated solely against the v2.0 design + the 13 planned slices; none need new planning. **(1) Case interlinking — INHERENT, not a separate system.** Cases are slices of one shared sim (world-level PersonRegistration + the shared faction/allegiance graph + shared territory), so interlinking emerges; the graph-analysis plan (add-the-graph-analysis-and-full-text-search-upgrade) is the traversal tool. **(2) Authored secret backbone + finite endgame — SUPERSEDED, dropped.** The v2.0 self-refreshing endless world (event-to-record-pipeline) + Integrity-based branching endings (detective-integrity plan) are the thematic payoff and conflict with an authored finite climax; and a "systemic secret" EMERGES from the sim anyway (the apex of the corruption graph + the deepest sealed record = the central White Whale), so authoring one is redundant. Pillar 4 is satisfied emergently. **(3) LLM renderer — KEPT but DEFERRED (polish, not load-bearing).** The template renderer already proves the loop (Pillar 1 keeps the LLM facts-only); v2.0's volume/variety (lazy realization + multi-cluster records + faction-flavored distortion) raised its value, so it stays a deferred polish plan, built after the sim+deduction loop is proven. **(4) Audio/visual (§13) — DEFERRED indefinitely.** Pure downstream production, orthogonal to every system designed; valueless to validating/building the engine; the last layer. Net: no new plans needed; §11.3 reconciled (the "secret" and interlinking are emergent, not authored; the Integrity arc + faction Standing drive the multiple endings). Reversible if a future design pass wants an authored north-star.|
|2026-06-04|**GCD v2.0 — the simulation-first pivot.** Promoted §14's *stretch* "simulation-first emergence" to the **core generation model**: a deterministic seeded **society-sim** simulates lives, factions, and crimes across the city's history (1920s → ~1950 present), **cold cases are the unsolved residue of that simulated past**, and faction actions **author the Registry's records** (Pillar 1's distortions now have *agents* — crime by muscle, the machine by authority, the press by circulation). Six reconciliations of locked decisions, each chosen with the user: **(1) Pillar 1 refined** — kept "the world never lies"/"solvable", but solvability is now **tiered**: every case has a *provable factual core*, a deliberate *marquee minority* are **underdetermined at the final link** (best theory always reachable, proof not — the city's Zodiacs), and **never a blank wall** (anti-"gotcha" holds; underdetermination ≠ unsolvable). **(2) Integrity axis** (§2.4) — an explicit **Integrity/corruption** meter for the detective (chosen over a standing-only reframe), mitigated against §2.4's anti-optimization rule by being a **liability/leverage** axis (not a virtue score, no optimal value, never shown as a grade): low Integrity = *being owned* (his own filings gain hidden fidelity; factions gain leverage). **(3) Names** — kept §4.2 **fictional analogues** (real St. Louis history as *inspiration* only; honors §1.4 sensitivity). **(4) Factions → autonomous** (§2.5) — the authored layer now **seeds** a running sim of **seven blocs** (Press, Political Machine, Organized Crime, Unorganized Crime, Law Enforcement, Reform/Civic, Business/Industry) → sub-factions → members, with **Allegiance portfolios** (public/secret, fluid by incentive−cost), a **relationships+Pacts** diplomacy graph (form/strain/break each tick; the Org↔Unorg turf war emerges here), and **active cover-up via the audit-log sensor net** (the observer effect); extends, not discards, the built faction-tension/requests plans. **(5) Era** → **mid-century (~1950)** present, cold cases reaching to the 1920s–30s (was "late 20th century"); analog texture holds. **(6) Server tier** (§14) — Registry becomes a Node/Fastify + **Prisma** API (SQLite dev → **Postgres** prod; **Apache AGE** faction graph; **RLS** access tiers), reconciling the not-started Registry plans with §14's prior pure-client app. Additive: decay/freshness, the **White Whale** archetype (§5.4), death-investigation + NIBRS record clusters, judicial/sealed-access. Discipline: every layer ships behind the solver gate; the MVP and Pillar 1 are never blocked by the sim. The §15 plan set is re-laid **foundation-first** (record substrate → Postgres migration → simulation → analysis/public surfaces). Reasoning: adopts the user's expansive simulation vision while preserving all four pillars via tiered solvability + authored seeds + the solver gate.|
